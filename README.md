...
**v3.5.1 "Asset Library & Polish"** transforms the Studio from infrastructure-perfect to content-rich, with a full professional asset library, fully functional post-process pipeline in Studio, complete camera, dynamic colored particles, signature 2006 particle tunnel, scene templates that actually create scenes, dedicated Asset Browser with drag & drop, and exhaustive UI audit. After v3.5.1, a novice user starts a complete VJ set in 30 seconds without manually creating a single node.
  - **Critical foundations (Lots A-C)**: `MeshGenerator::registerDefaults()` registers 7 canonical procedural meshes at startup (torus/cylinder/cone/plane_1m/torusknot/cube_1m/bbfx_plane), dragon textures extracted (BODY/HEAD4/LEGS), `Examples/Robot` material created (r2skin.jpg), 5 functional skybox materials (Stormy/Morning/Evening/EarlyMorning/CloudyNoon with extracted cubemaps). **PostProcessStack**: new ping-pong RTT pipeline architecture independent of OGRE Compositor — full bypass of CompositorManager via Rectangle2D + `_render()` direct (avoids ImGui FBO corruption). PostProcessNode DAG (violet category), CompositorNode preserved as placeholder for v3.5 backward compat. Migration of 16 BBFx + 6 OGRE effects (Bloom/DOF single-pass, Embossed/OldTV/Glass standalone). 22 effects in catalogue.
  - **Enhanced nodes (Lots D-F)**: **CameraNode** refactored with 6-mode dispatch (orbit/fly_through/shake/dolly_zoom/track/crane), smoothstep+Slerp transitions, DAG ports `target.x/y/z`/`shake_intensity`/`transition_time`/`fly_speed`/`damping`. **ParticleNode** extended with 8 DAG ports (color.r/g/b/a + particle_size + velocity + lifetime + emission_rate), multiplier mode preserving template via `mOrigWidth/Height/VelMin/Max`. 10 BBFx templates (Fire/ElectricArc/Confetti/MagicDust/NeonTrail/Bubbles/LaserBeam/Galaxy/MatrixRain/**ParticleTunnel signature 2006**) + camera 50u recalibration (sizes x3-5). Preset deduplication (Geometry 6→3, Color 4→2+2 GPU), backward-compat aliases, fixes for particle_symphony/material_cycle/starwars_tribute.
  - **GPU shaders (Lots G-H)**: 5 vertex shaders (explode, inflate, spiral, audio_pulse, flatten), 8 fragment generators (glitch_block, julia, fire, fbm_warp, cellular_automata, hexagonal, moire, waveform), activation of 4 orphan shaders (wave_gpu/perlin_gpu vert + datamosh/pixel_sort frag). 7 post-process fragments (halftone, cross_hatch, oil_paint, color_lut, radial_blur, feedback_zoom, pp_motion_trail). **Feedback prevFrame RTT** added to PostProcessStack (3rd pre-allocated RTT, auto-detection `uniform sampler2D prevFrame`). **ShaderFxNode vec2/3/4 support** (extended regex, decomposition into `.x/.y/.z/.w` ports, `setNamedConstant Vector2/3/4`). Catalogue 22 → **29 effects**.
  - **Complete content (Lots I-K)**: 14 functional scene templates (empty preserved, bonneballe_basic, ambient/hiphop/house/techno/dubstep/dnb by music genre, beat_machine, particle_show, shader_lab, audio_reactive, full_performance ≥ 8 nodes, video_mix). 14 enriched presets (CompositionNode `build()` format with nodes/links) — bonneballe_classic flagship, tunnel_party, fractal_explorer, neon_geometry, fluid_dreams, glitch_art, retro_arcade, frequency_landscape + 6 audio-reactive. 5 new procedural meshes (mobius, lissajous, helix, diamond, star3d), 8 VJ materials (BBFx/Chrome, Neon, GlassVJ, Wireframe, Hologram, Emissive, Gradient, Reflective).
  - **Asset Browser + Polish (Lots L-M)**: new `AssetBrowserPanel` (~450 lines) with 9 sidebar categories (Meshes/Textures/Materials/Shaders/Particles/Effects/Cameras/Presets/Templates), 64x64 grid + list view, instant search, AND multi-select tags, persisted favorites, info tooltip, **drag&drop with 8 typed payloads** (MESH_NAME/TEXTURE_NAME/SHADER_NAME/PARTICLE_NAME/MATERIAL_NAME/PRESET_NAME/COMPOSITOR_NAME/TEMPLATE_NAME) + double-click, ResourceEnumerator extended (listPresets/listTemplates/listPostProcessEffects), Ctrl+Shift+A. Exhaustive tests, version bump v3.5.1, USAGE.md updated. **190 PASS / 0 FAIL** at initial release.
  - **Phase 6 stabilization (37 hotfixes I-1670→I-1706)**: complete Asset Browser drag&drop audit (4 bugs + root cause "isolated nodes don't receive update()" → explicit post-create call), particle template recalibration (camera 50u), **root cause fix ParticleNode overwrites template** (defaults -1.0, multiplier mode), Aureola_tweak fix, Perlin clone material transfer (sub-entity by sub-entity), HBL_DISCARD→HBL_NORMAL preserves UVs, ViewportPicker `bbfx_target_dag` tag maps clones to DAG, **mFxHidden flag** separate from mUserVisible, **`addTexCoordsIfMissing()` spherical UVs** (fix Perlin clone black render with texture on geosphere without TEXCOORD), **TextureNode lighting modes** (unlit/lit/emissive) + global setting `defaultLightingMode`, **fix ParticleNode color override** (dynamic color affector removal + TVC_DIFFUSE + restore on disable), TextureNode multi-textures (priority by connection sequence), **viewport camera persistence** (OrbitState in extraJson["camera"]), MasterView output toggle (visible flag + ShowWindow + skip render), PresetBrowser Assets/Quick Access removed (redundant), **standalone EffectRackPanel** (~420 lines) with MIDI/Gamepad/Keyboard learn per row + LED feedback + undoable bypass + serialization `extraJson["effectRack"]` + F9 shortcut.
...
### BBFx Asset Library & Polish (v3.5.1)
- **MeshGenerator::registerDefaults()** — 7 canonical procedural meshes registered at startup (torus.mesh, cylinder.mesh, cone.mesh, plane_1m.mesh, torusknot.mesh, cube_1m.mesh, bbfx_plane.mesh) + 5 new (mobius, lissajous, helix, diamond, star3d) — 32 accessible meshes total
- **PostProcessStack** (`src/fx/PostProcessStack.h/.cpp`) — ping-pong RTT pipeline independent of OGRE Compositor, Rectangle2D + `_render()` direct (bypasses CompositorManager FBO corruption), 3rd `prevFrame` RTT for feedback effects, auto-detection `uniform sampler2D prevFrame`
- **PostProcessEffect + PostProcessNode** — DAG node "PostProcess" category violet, ParamSpec compositor/enabled/order, `getAvailableEffects()` catalogue 29 effects (16 BBFx migrated + 6 OGRE adapted single-pass + 7 new)
- **CameraNode 6 modes** — orbit/fly_through/shake/dolly_zoom/track/crane dispatch, smoothstep+Slerp+lerp transitions, DAG ports target.x/y/z, shake_intensity, transition_time, fly_speed, fly_axis, damping, crane_amplitude, crane_speed
- **ParticleNode 8 DAG ports** — color.r/g/b/a + particle_size + velocity + lifetime + emission_rate, defaults -1.0 preserve template, multiplier mode via mOrigWidth/Height/VelMin/Max, dynamic color affector removal + TVC_DIFFUSE for color override
- **10 BBFx particle templates** — Fire/ElectricArc/Confetti/MagicDust/NeonTrail/Bubbles/LaserBeam/Galaxy/MatrixRain/**ParticleTunnel signature 2006** (Ring emitter r=30, vel 80-120 Z- toward camera, ports ring_radius/ring_speed/ring_density)
- **15 GPU shaders** — 5 vertex (explode, inflate, spiral, audio_pulse, flatten) + 8 fragment generators (glitch_block, julia, fire, fbm_warp, cellular_automata, hexagonal, moire, waveform) + 7 post-process (halftone, cross_hatch, oil_paint, color_lut, radial_blur, feedback_zoom, pp_motion_trail) — 58 active shaders total
- **ShaderFxNode vec support** — extended regex `uniform (vec[234]) (\w+)` decomposes vec2/3/4 into `.x/.y/.z/.w` ports, setNamedConstant Vector2/3/4
- **ShaderFxNode BPM fallback** — auto-detection bass/mid/high audio uniforms, tempo-pulsed exp envelope when port not connected (quarters/eighths/sixteenths), disabled when AudioCaptureNode connected
- **8 VJ materials** — BBFx/Chrome (env_map spherical), BBFx/Neon (emissive), BBFx/GlassVJ (alpha_blend), BBFx/Wireframe (polygon_mode), BBFx/Hologram (scene_blend add scanlines), BBFx/Emissive, BBFx/Gradient (vertex_colour), BBFx/Reflective + 5 skybox materials (Stormy, Morning, Evening, EarlyMorning, CloudyNoon)
- **AssetBrowserPanel** (`src/studio/panels/AssetBrowserPanel.h/.cpp`) — 9 categories sidebar, 64x64 grid + list view toggle, instant search, AND multi-select tags, persisted favorites, info tooltip, drag&drop with 8 typed payloads (MESH_NAME/TEXTURE_NAME/SHADER_NAME/PARTICLE_NAME/MATERIAL_NAME/PRESET_NAME/COMPOSITOR_NAME/TEMPLATE_NAME) + double-click actions, Ctrl+Shift+A
- **ResourceEnumerator extended** — listMeshes (incl. procedural via MeshManager iterator), listTextures, listMaterials, listShaders, listParticleTemplates, listPostProcessEffects, listPresets (scan lua/presets/), listTemplates (scan lua/templates/)
- **EffectRackPanel** (`src/studio/panels/EffectRackPanel.h/.cpp`) — standalone panel (~420 lines) extracted from PresetBrowserPanel, undoable bypass via SetEnabledCommand, LED-style feedback (green/red), MIDI/Gamepad/Keyboard learn per row with badges, F9 shortcut, serialization extraJson["effectRack"]
- **TextureNode lighting modes** — `mLightingMode` ENUM {unlit, lit, emissive}, material name includes mode for proper recreation, global setting `defaultLightingMode` in SettingsManager (Settings dialog combo), ambient 0.3 → 1.0 (BaseWhite-aligned)
- **TextureNode priority by sequence** — `sApplySeqCounter` monotonic + `mApplySeq` per-target map, last connected wins on re-enable
- **SceneObjectNode mEntityVersion** — monotonic counter incremented on entity (re)creation, FX nodes (ColorShiftNode, ShaderFxNode) compare by version instead of raw pointer (avoids dangling pointer false negatives on memory address reuse)
- **SceneObjectNode mFxHidden** — flag separate from mUserVisible, `setVisible(vis && mEnabled && mUserVisible && !mFxHidden)`, prevents FX hide being overwritten each frame, isNodeVisible() ignores mFxHidden
- **SceneObjectNode mCurrentMaterial** — tracks applied material without overwriting per-submesh embedded materials (e.g. ogrehead.mesh Skin/Eyes/Tusks)
- **SoftwareVertexShader::addTexCoordsIfMissing()** — detects missing VES_TEXTURE_COORDINATES (e.g. geosphere4500), creates expanded buffer (vertexSize + 8 bytes), generates spherical UVs `u = 0.5 + atan2(z,x)/(2π)`, `v = 0.5 - asin(y)/π` — fixes Perlin clone black render with textures
- **PerlinVertexShader UV-preserving** — HBL_DISCARD → HBL_NORMAL on interleaved buffer (POS+NORMAL+UV), preserves UVs and tangents during write-back
- **ViewportPicker bbfx_target_dag tag** — UserObjectBindings on Perlin clones, fast-path lookup in findDAGNodeForEntity()
- **ColorShiftNode dynamic target resolution** — port `entity` added (multiLink, standard FX pattern), resolveTarget() via Animator::getSourceNodes(), applyToEntity()/detachFromEntity() capture-restore originals, factory simplified
- **ViewportCameraController OrbitState** — getOrbitState/setOrbitState, serialized in extraJson["camera"] (yaw, pitch, distance, centerX/Y/Z), backward-compatible
- **OutputManager visible flag** — per-output visible toggle (Win32 ShowWindow / Unix SDL_ShowWindow), skip render/blit/swap/texture sharing when !visible, lazy creation
- **MasterViewPanel toggle** — Show/Hide button per output, dimmed thumbnail (alpha 0.35), black background when hidden
- **Persistent docking layout** — DockBuilder only executes if no saved layout (DockBuilderGetNode == nullptr), SaveIniSettingsToDisk on saveProject, Asset Browser added to default layout (tabbed with Timeline)
- **27 panels UI audit (Lot N)** — double status bar removed, auto-save complete (6 sections added: zone snapshots, outputs, surface map, network, effect rack, MIDI bindings), SetEditor playback (cut/crossfade/fade transitions, beat accumulation), SurfaceEditor 8 resize handles, shortcuts dialog corrected, MIDI bindings serialized in project, Ctrl+N/Ctrl+D aligned, Console history Up/Down, MidiActivity device filter, MidiMapping Clear All confirmation, dead code removed
- **14 scene templates** — empty (preserved), bonneballe_basic, ambient (BPM 70), hiphop (90), house (124), techno (135), dubstep (140), dnb (172), beat_machine (128), particle_show (140), shader_lab (120), audio_reactive (0), full_performance (128 ≥ 8 nodes), video_mix (120) — using `dbg.create_with_param()` for deferred mesh injection
- **93 active presets** + 6 backward-compat aliases (color_shift, monochrome_fade, perlin_pulse, perlin_breath, geosphere_explode, vertex_noise) — 14 enriched compositions in CompositionNode format (bonneballe_classic flagship, tunnel_party, fractal_explorer, neon_geometry, fluid_dreams, glitch_art, retro_arcade, frequency_landscape, audio-reactive)
- **resources/LICENSES.md** — attribution for OGRE MIT assets, BBFx originals, shader references
